...
{
#  define MAX_REG_WRITE_SIZE 2464 /* PPC64 w/ QPX */
   ...
   if (0 == VG_(strcmp)(arg, "--track-origins=yes")) {
	...
	if VG_BOOL_CLO(arg, "--partial-loads-ok", MC_(clo_partial_loads_ok)) {}
      ...
      for (i = 0; i < ignoreRanges.used; i++) {
         ...
         /* Entirely arbitrary limit, for sanity checking.  On 32-bit
            targets, make it 64M.  On 64-bit targets, let's say 2048
            chunks of 4G. */
         Addr limit = sizeof(Addr) == 4 ? (Addr)0x4000000ULL 
                                        : (Addr)(2048ULL * (1ULL << 32));
         if (e <= s) {
            VG_(message)(Vg_DebugMsg, 
