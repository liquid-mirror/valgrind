...
Int VG_(write) ( Int fd, const void* buf, SizeT count)
{
   Int    ret;
#  if defined(VGO_linux)
...
Int VG_(mkstemp) ( HChar* part_of_name, /*OUT*/HChar* fullname )
{
   ...
   /* On BGQ, fold in extra randomness since getpid/getppid return the
      same value for all processes in an MPI set, and so are useless.
      FIXME: we should really also use something that differentiates
      processes based on placement. */
#  if defined(VGPV_ppc64_linux_bgq)
   { SysRes res;
     struct vki_timeval tv_now;
     res = VG_(do_syscall2)(__NR_gettimeofday, (UWord)&tv_now, (UWord)NULL);
     vg_assert(!sr_isError(res));
     UInt noise = (UInt)tv_now.tv_sec ^ (((UInt)tv_now.tv_usec) << 3);
     seed ^= noise;
   }
#  endif
