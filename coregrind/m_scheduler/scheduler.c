...
#  if defined(VGA_ppc64)
   /* Make sure the QPX bits are at least 8 aligned. */
   vg_assert(VG_IS_8_ALIGNED(& tst->arch.vex.guest_QPX0_1));
#  endif
...
#        endif
      ...
      case VEX_TRC_JMP_WAITRESVN:
	 VG_(release_BigLock)(tid, VgTs_Yielding, 
                                   "VG_(scheduler):waitrsv");
	 /* ------------ now we don't have The Lock ------------ */
         __asm__ __volatile__("waitrsv");
	 VG_(acquire_BigLock)(tid, "VG_(scheduler):waitrsv");
	 /* ------------ now we do have The Lock ------------ */
         break;
