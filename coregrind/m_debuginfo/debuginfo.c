...
UInt VG_(seginfo_num_locs) ( const DebugInfo *di )
{
   return di->loctab_used;
}

Bool VG_(seginfo_locN_addr)( const DebugInfo *di, UInt n, Addr* addr )
{
   if (n < 0 || n > di->loctab_used)
      return False;
   else {
      *addr = di->loctab[n].addr;
      return True;
   }
}

Bool VG_(seginfo_locN_size)( const DebugInfo *di, UInt n, UInt* size)
{
   if (n < 0 || n > di->loctab_used)
      return False;
   else {
      *size = di->loctab[n].size;
      return True;
   }
}

Bool VG_(seginfo_locN_line)( const DebugInfo *di, UInt n, UInt* line)
{
   if (n < 0 || n > di->loctab_used)
      return False;
   else {
      *line = di->loctab[n].lineno;
      return True;
   }
}

Bool VG_(seginfo_locN_filename)( const DebugInfo *di, UInt n, Char** filename)
{
   if (n < 0 || n > di->loctab_used)
      return False;
   else {
      *filename = di->loctab[n].filename;
      return True;
   }
}

Bool VG_(seginfo_locN_dirname)( const DebugInfo *di, UInt n, Char** dirname)
{
   if (n < 0 || n > di->loctab_used)
      return False;
   else {
      *dirname = di->loctab[n].dirname;
      return True;
   }
}
