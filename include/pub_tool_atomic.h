...
/*--------------------------------------------------------------------*/
/*--- Atomic primitives.                         pub_tool_atomic.h ---*/
/*--------------------------------------------------------------------*/
...
#ifndef __PUB_TOOL_ATOMIC_H
#define __PUB_TOOL_ATOMIC_H

//--------------------------------------------------------------------
// PURPOSE: Provides atomic operations primitives.
//--------------------------------------------------------------------
...
#if defined (VGA_x86)
#include "priv_atomic_x86.h"
#elif defined (VGA_amd64)
#include "priv_atomic_amd64.h"
#else
  Missing architecture in pub_tool_atomic.h
#endif
...
/* Atomically *mem = 0 and return the old value of *mem.  */
#ifndef atomic_zero_val
# define atomic_zero_val(mem) \
  ({ __typeof (*(mem)) __atg20_old;					      \
     __typeof (mem) __atg20_memp = (mem);				      \
									      \
     do									      \
       __atg20_old = (*__atg20_memp);					      \
     while (__builtin_expect						      \
	    (atomic_compare_and_exchange_bool_acq (__atg20_memp,	      \
						   0,                         \
						   __atg20_old), 0));	      \
									      \
     __atg20_old; })
...
#endif	/* __PUB_TOOL_ATOMIC_H */

/*--------------------------------------------------------------------*/
/*--- end                                        pub_tool_atomic.h ---*/
/*--------------------------------------------------------------------*/
